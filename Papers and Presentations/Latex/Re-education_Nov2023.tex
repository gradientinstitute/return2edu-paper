...
Retraining in later-life is pursued by adults as a transition pathway into a job, a promotion or a career change \citep{oecd2016}. Technological change and
automation may require some individuals -- especially middle-income workers performing routine tasks -- to retrain \citep{autor2008,acemoglu2011}. Also at the lower end of the
income-distribution, such as among welfare recipients, governments continue to promote retraining opportunities. Adults may be made worse-off economically, however, when they re-enter formal education because of the significant direct and indirect costs associated with formal study and if the skills they gain are divorced from those demanded in the labour market \citep{midtsundstad2019review}. 

We aim to assess the long-term impact of obtaining a formal educational degree on earnings,
focusing on mature-age students. Previous researchers have estimated positive and significant wage premiums for `younger students', ranging between 5 and 13\% \citep{angrist1991,harmon2003,machin2006} or even higher than 15\% as in the case of \cite{harmon1995}. Among those who look at mature-age learners, the focus has been on those studying at a specific educational institution, such as at a community or training college \citep{jacobson2005, chesters2015, 
zeidenberg2015, polidano2016, xu2016, belfield2017, dynarski2016, dynarski2018,
mountjoy2022}, those who undergo a government training program (and are relatively low-productivity workers) \citep{ashenfelter1978, ashenfelter1985, bloom1990, leigh1990, raaum2002, jacobson2005, STENBERG200854, card2018, knaus2022}, or those who return to high school \citep{bennett2020second}. Again, the impacts are estimated to be strong and positive, particularly for female students and once modelling takes into account the different pre-study earnings-growth profiles of students and non-students. The majority of studies look at the impacts in the short to medium term.

We complement the above two strands of the literature by estimating the long-run returns for adults (aged 25 or above) undertaking a broader remit of formal degrees (post-graduate degrees, training certificates, diplomas etc) and spanning different institutions at which the study took place. This means we analyse the effects for a more diverse group of students, which then allows us to measure how the impact of increasing human capital may vary across a broader range of sub-populations.

A key challenge in estimation is that factors that enable mature-age learners to pursue and complete a qualification may also be precursors to later-life success. Moreover, the
...
work to intensively control for many confounding factors (and non-linearities in these factors), as well as to discover potentially new sources of treatment heterogeneity \citep{athey2017, knaus2021}. For high-dimensional and complex datasets such as the one we use in this
research, these methodological abilities are crucial in reducing bias from model mis-specification and confounding (e.g.~selection into treatment), and reducing variance from correlation/collinearity.


We adapt ML tools for causal inference purposes. Standard off-the-shelf ML models are better suited to predictive purposes. When
...
We recognise that, as with all statistical models, we make assumptions when we use ML techniques for causal inference, and these need to be tested. One key assumption is that the controls included in the ML models sufficiently account for selection into treatment. We undertake a replication exercise where we compare the results of the ML model with that of baseline models, using Ordinary Least Squares (OLS) and Fixed Effects. We also contrast the selected control variables in the ML model with those that were manually selected in previously published work \cite{chesters2015}, and comment on the potential biases from manual variable selection. We have chosen this published work because it uses the same data (HILDA) and examines the same topic.
...
and on their attitudes and preferences. We also account for pre-study trajectories. Access to this broad range of
...
variables are consistently selected across different ML models (which includes linear and non-linear model classes) and
...
We define further education as an individual who obtains a further degree in a formal, structured educational program. These programs must be delivered by a certified training, teaching or research institution. Thus, we do not analyse informal online degrees (such as Coursera degrees). We also do not consider on-the-job training as obtaining further education.
...
purview of machine learning methods.
...
the T-learner approach because it can reduce mis-specification error either
...
reduce mis-specification error either through a correctly specified propensity
...
In addition, we show evidence that models which allow for more flexible functional-form specifications lead to differences in the ATE. Within our ML models, the GBR tree ensemble tended to perform better (in terms of the nested cross validation results) compared to the linear-based models. The former yielded a slightly smaller ATE compared to the LASSO and Ridge results, for example, and they were also consistent with results from the Bayesian Causal Forest.
...
Similar treatment effects apply to those with and without resident children,
...
\caption{Selecting and Evaluating Model Class}
  \label{fig:exp_model}
    \includegraphics[width=\linewidth]{_figures/explainer1.pdf}
\end{figure}
...
\caption{Generating Uncertainty Parameters}
  \label{fig:exp_param}
    \includegraphics[width=\linewidth]{_figures/explainer2.pdf}
\end{figure}
