...
We approach filling the missing data indirectly with three types of ML models. Ther are: the T-Learner, Doubly Robust and Bayesian Linear models. 
...
The T-learner is a two-step approach where the conditional mean functions $\mu _1(x) = E[Y_1, X_i = x]$ and $\mu _0(x) = E[Y_0, X_i = x]$ are estimated separately with any generic machine learning algorithm.
...
\begin{align*}
\{ \lambda_0, \lambda_t, \lambda_x, \lambda_{tx} \} \sim Gamma(1,1) \\
\sigma^2 \sim Gamma(1,1) \\
w_0 \sim N(0,\lambda_0) \\
w_t \sim N(0,\lambda_t) \\
\textbf{w}_x \sim N(0,\lambda_x \textbf{I}_d) \\
\textbf{w}_{tx} \sim N(0,\lambda_{tx} \textbf{I}_d) \\
\end{align*}
 
The advantage of this model is that it is free to share information between both treatment and control response surfaces during posterior inference (model fitting). This is unlike a T-learner, which uses entirely separate models and data to estimate the control and response surfaces. We believe this information sharing property of the Bayesian model helps reduce [error/variance?] in effect estimation from model extrapolation in circumstances where there is little overlap in the support of $P(x|t=treatment)$ and $P(x|t=control)$, which is an issue with the T-learner approach. This information sharing comes from the fact that the $\lambda^*$ hyper-parameters are all from a shared distribution, which incorporates information from $t_i$, $x_i$ and the interaction $t_i \cdot x_i$ to update the posterior over these parameters. For model inference, we use the no U-turn MCMC sampler \citep{hoffman2014} in the pyro software package \citep{bingham2019}. The choice of a Gamma(1, 1) prior is partially motivated by the fact that we standardise the inputs, $x$, to the model to have zero mean and unit variance and $t \in \{0, 1\}$. We also tried a sparsity inducing Laplacian prior over weights, $w$, but we did not find a substantial difference from using Normal priors.
...
Gaussian Process	 & 	5441	 & 	53.96	 & 	[8.36, 99.56]	 \\
Hierarchical Bayesian Linear Regression	 & 	5441	 & 	45.74	 & [-9.50, 113.53]  \\
\bottomrule
\end{tabular}
\par\medskip
